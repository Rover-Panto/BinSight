...
- Candidate laptop-training artifact: `artifacts/yolo11n_recycling-8_best.pt` (**laptop prototype only; not for Grove Vision AI V2**)
...
This `.pt` file is supplied for reproducibility of the laptop prototype only. It must not be flashed to or used as a Grove Vision AI V2 camera model. It is not an approved deployment model: it has no completed dataset licence/provenance record, per-class held-out evaluation, or Grove Vision AI V2 `*_int8_vela.tflite` export/flash evidence.
