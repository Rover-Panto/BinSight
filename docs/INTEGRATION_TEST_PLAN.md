...
Owner clarification, 29 August: prioritize a working controlled demo, not production readiness. Review blockers must affect the demonstrated workflow, data preservation or basic physical safety. Defer general-purpose infrastructure and model hardening when the demo can use a known artifact and tested environment. PR4 `28509cc` is accepted for integration testing under [these demo conditions](PR4_REVIEW_2026-08-29.md#demo-acceptance); the broad diagnostic failures remain recorded, not converted into passes. Full-system acceptance still requires the route/return workflows and relevant bench tests to work.
...
| G04 Forecast semantics | PR4/PR1 | Use the reviewed fixed bundle and tested environment. Preserve the 90% service-threshold meaning; unsupported probabilities select the named non-ML fallback. Normalize observation/receipt/decision timestamps before PR4; bad, missing, stale and duplicate-only histories produce explicit supported states without NaN or invented zero fill. Verify the route smoke test and historical cutoff for the actual demo scenario. Arbitrary-bundle/version-matrix hardening and retraining automation are deferred. |
